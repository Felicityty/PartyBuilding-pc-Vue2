    ...
    <el-dialog title="新增" :visible.sync="addFormVisible" width="35%" @close="handleClose">
      <el-form ref="addFormRef" :model="form" :rules="addRules" label-position="right" label-width="35%">
        ...
        <el-form-item label="班级名称:" prop="classGradeName">
          <el-select v-model="form.classGradeName" placeholder="请选择" style="width: 70%" @change="$forceUpdate()">
...
<script>
import { addAcademicList } from '@/api/academic'
  ...
  methods: {
    ...
    handleConfirm() {
      this.$refs.addFormRef.validate((valid) => {
        ...
        this.loading = true
        addAcademicList(this.form).then((res) => {
          this.loading = false
          if (res.code === 200) {
