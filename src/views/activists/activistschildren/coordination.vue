  ...
  <div>
    ...
    <el-card>
      <TopBar class-name="top-div-left-menu-five">
        <!-- 左菜单 -->
        <template #btnLeft>
          <AddNew category="5" @confirmAdd="confirmAdd" />
          <BulkImport category="5" @handleSearch="handleSearch" />
          ...
          <DownModule file-name="1d666711-c0af-4645-9cd6-6612afe4566f.xlsx" original-filename="发展能力-组织协调模板.xlsx" />
          <BulkExport file-name="发展能力-组织协调.xlsx" :headers="headers" category="4" />
      ...
      <coordinationTable ref="coordinationRef" />
...
import coordinationTable from '@/components/coordination/coordinationTable.vue'
...
export default {
  ...
  methods: {
    ...
    handleSearch(clearSelection) {
      this.$nextTick(() => {
        this.$refs.coordinationRef.fetchData(this.queryInfo, clearSelection)
      })
    }
