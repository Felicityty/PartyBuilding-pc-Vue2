  ...
  <div>
    ...
    <el-card>
      ...
      <top-bar class-name="top-div-left-menu-five">
        ...
        <template #btnLeft>
          <!-- 新增 -->
          <AddNew category="3" @confirmAdd="confirmAdd" />
          <!-- 批量导入 -->
          <BulkImport category="3" @handleSearch="handleSearch" />
          <!-- 下载模板 -->
          <DownModule file-name="dae59edb-f8c0-4286-aeb2-f5e6bf6c2863.xlsx" original-filename="发展能力-个人荣誉模板.xlsx" />
          <!-- 批量导出 -->
          <BulkExport file-name="发展能力-个人荣誉.xlsx" :headers="headers" category="3" />
          <!-- 批量删除 -->
...
export default {
  ...
  methods: {
    ...
    handleSearch(clearSelection) {
      this.$nextTick(() => {
        this.$refs.honortable.fetchData(this.queryInfo, clearSelection)
      })
    },
    // 重置搜索条件
    handleReset() {
